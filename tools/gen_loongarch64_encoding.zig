...
pub fn main() anyerror!void {
    ...
    defer zig_dir.close();
    const out_file = try zig_dir.createFile("src/arch/loongarch64/Instruction.zig", .{});
    defer out_file.close();
    const out_writer = out_file.writer();

    // TODO: include commit hash of loongarch-opcodes
    try out_writer.writeAll("// This file is auto-generated by tools/gen_loongarch64_encoding.zig\n\n");
    ...
    for (descs.items) |*desc| {
        ...
        try out_writer.print(" => .{{ 0x{x:0>8}, .{s} }},\n", .{ desc.word, format_str });
    }
    try out_writer.writeAll(
        \\        };
        \\    }
    ...
    var format_iter2 = formats.keyIterator();
    while (format_iter2.next()) |format| {
        var format_buf = std.ArrayList(u8).init(arena);
        defer format_buf.deinit();
        try OpcodeDesc.stringifyFormat(format, format_buf.writer());
        const format_str = format_buf.items;
        if (std.mem.eql(u8, format_str, "EMPTY")) {
            ...
            for (format, 0..) |slot, slot_i| {
                ...
                switch (slot) {
                    .none => unreachable,
                    .reg => |reg| {
                        try out_writer.print("r{}: Register", .{reg.index});
                    },
                    .imm => |imm| {
                        try out_writer.print("imm{}: {c}{}", .{
                            imm_i,
            ...
            for (format, 0..) |slot, slot_i| {
                ...
                switch (slot) {
                    ...
                    .reg => |reg| {
                        ...
                        if (reg_off == 0) {
                            try out_writer.print("data.r{}.enc()", .{reg.index});
                        } else {
                            try out_writer.print("(@as(u32, data.r{}.enc()) << {})", .{ reg.index, reg_off });
                        }
                    },
                    .imm => |imm| {
                        const imm_off = imm.index.offset();
                        if (imm_off == 0) {
                            try out_writer.print("@as(u32, @intCast(@as(u{}, @bitCast(data.imm{}))))", .{ imm.length, imm_i });
                        } else {
                            try out_writer.print("(@as(u32, @as(u{}, @bitCast(data.imm{}))) << {})", .{ imm.length, imm_i, imm_off });
                        }
                        imm_i += 1;
                    },
                }
...
fn usageAndExit(arg0: []const u8, code: u8) noreturn {
    ...
    stderr.writer().print(
        ...
        \\
        \\Updates src/arch/loongarch64/Instruction.zig from https://github.com/loongson-community/loongarch-opcodes.git.
        \\
